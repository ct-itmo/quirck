...
def parse_user(user_info: dict[str, Any]) -> dict[str, Any]:
    ...
    name = (
        f"{user_info.get('family_name')} {user_info.get('given_name', '')} {user_info.get('middle_name', '')}".strip()
        or user_info["name"]
    )

    return {"id": user_info["isu"], "name": name, "group": group}
...
async def sso_callback(request: Request) -> Response:
    try:
        token = await sso_client.process_code_flow(
            request, redirect_uri=str(request.url_for("auth:callback"))
        )
    except OAuthException as exc:
        logger.info("Authorization failed: %s", exc)
        return RedirectResponse(
            f"{request.url_for('auth:failed')}?error=oauth", status_code=303
        )
    ...
    if "isu" not in user_info:
        return RedirectResponse(
            f"{request.url_for('auth:failed')}?error=isu", status_code=303
        )
    ...
    if user is None:
        ...
        if not can_register and len(ALLOWED_GROUPS) > 0:
            can_register = (
                "*" in ALLOWED_GROUPS
                or "groups" in user_info
                and any(
                    group["name"] in ALLOWED_GROUPS for group in user_info["groups"]
                )
            )

        if not can_register:
            return RedirectResponse(
                f"{request.url_for('auth:failed')}?error=group", status_code=303
            )

    statement = insert(User).values([user_record])
    statement = statement.on_conflict_do_update(
        index_elements=[User.id],
        set_=dict(name=statement.excluded.name, group=statement.excluded.group),
    ).returning(User)

    user = (
        await database.scalars(statement, execution_options={"populate_existing": True})
    ).one()
...
async def sso_login_failed(request: Request) -> Response:
    ...
    return TemplateResponse(
        request, "auth_error.html", {"error": error}, status_code=403
    )
...
async def sso_logout(request: Request) -> Response:
    if "origin_user_id" in request.session:
        logger.info(
            "User %s is logged out from %s",
            request.session["origin_user_id"],
